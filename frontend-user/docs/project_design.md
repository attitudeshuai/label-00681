...
- **模块化设计**：核心游戏逻辑（移动、状态管理、撤销/重置）与 UI 渲染层分离。尽管基于项目规模采用了单文件组件 (`App.vue`) 模式，但代码内部结构清晰，通过 `Init`, `State`, `Actions`, `Events` 等区域注释严格划分，便于后续维护与扩展。
...
├── src/
│   ├── App.vue              # 核心应用组件 (包含游戏引擎、UI逻辑、模板与样式)
│   ├── main.js              # Vue 应用入口挂载
